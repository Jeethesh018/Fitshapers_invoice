...
export default function FormPanel({ step, data, setData, setStep, onDownload, onSend, sending }) {
      ...
      <AnimatePresence mode="wait">
        {step === 1 && (
          <motion.div key="step1" {...slide} transition={{ duration: 0.35 }}>
            ...
            <input className="input mb-4" value={data.name} onChange={(e) => setField('name', e.target.value)} placeholder="e.g. Riya Sharma" />
            <label className="label">Contact Number</label>
            <input className="input mb-6" value={data.contact} onChange={(e) => setField('contact', e.target.value)} placeholder="e.g. +91 98765 43210" />
            <button onClick={() => setStep(2)} className="w-full rounded-xl bg-ember-gradient px-5 py-3 font-semibold text-white transition hover:scale-[1.01]">
              Continue to Package
            </button>
          </motion.div>
        )}
        ...
        {step === 2 && (
          <motion.div key="step2" {...slide} transition={{ duration: 0.35 }}>
            ...
            <input className="input mb-4" value={data.packageType} onChange={(e) => setField('packageType', e.target.value)} placeholder="Personal Training - 3 Months" />
            ...
            <input type="number" className="input mb-6" value={data.amount} onChange={(e) => setField('amount', e.target.value)} placeholder="12000" />
            <div className="flex gap-3">
              <button onClick={() => setStep(1)} className="w-1/3 rounded-xl border border-zinc-600 px-4 py-3">Back</button>
              <button onClick={() => setStep(3)} className="w-2/3 rounded-xl bg-ember-gradient px-4 py-3 font-semibold text-white">Go to Preview</button>
            </div>
          </motion.div>
        )}
